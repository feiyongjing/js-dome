...
const student = Vue.extend({
    ...
    // name:'组件的name属性设置开发者工具中显示的组件名称，但是无法影响组件注册和在html中使用时的组件名称',
...
let NonSingleFileComponent = new Vue({
    el: ".NonSingleFileComponentVm",
    ...
    // 控制台vm.$children会看到vm下的组件，而组件下的$children会看到向下嵌套的组件
    components: {
...
// school和student组件本质都是一个名为VueComponent的构造函数，VueComponent不是程序员定义的，是Vue.extend生成的。每次调用Vue.extend返回的
// 都是一个全新的VueComponent
// 当html或者是模板的<school/>或者是<school></school>被Vue解析时会创建school组件的实例对象，
// 即Vue执行 new VueComponent(options)生成VueComponent实例对象(简称vc)
// 在组件配置中：data函数、methods中的函数、watch中的函数、computed中的函数，它们的this都是vc
// 在new Vue(options)配置中：data属性、methods中的函数、watch中的函数、computed中的函数，它们的this都是vm
